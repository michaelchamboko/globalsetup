...
description: Use when long-horizon work resumes in fresh context or the current task context becomes unreliable.
---
...
4. In the fresh context, read `AGENTS.md`, run BuildRunner `validate` and `next`, inspect Git status, and load only the returned `context_files` plus relevant GitNexus context.
